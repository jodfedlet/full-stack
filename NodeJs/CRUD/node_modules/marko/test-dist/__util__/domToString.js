...
function getNodeType(node) {
    return node.nodeType || node.aA_;
}

function getNodeValue(node) {
    return node.aF_ || node.nodeValue;
}

function getFirstChild(node) {
    return node.R_ || node.firstChild;
}

function getNextSibling(node) {
    return node.at_ || node.nextSibling;
}
...
function vdomToHTML(node, options) {
    ...
    function serializeElHelper(el, indent) {
        var tagName = el.nodeName || el.ay_;

        var elNamespaceURI = el.namespaceURI || el.bz_;
        ...
        var attributes = el.attributes || el.by_;
        var attributesArray = [];
        var attrName;
        ...
        if (tagName.toUpperCase() === "TEXTAREA") {
            html += indent + "  VALUE: " + JSON.stringify(ltrim(el.value || el.aH_)) + "\n";
        } else {

...
module.exports = function autotest(fixturesName, run) {
    ...
    describe(path.join(suiteName, fixturesName), () => {
        ...
        if (typeof run === "function") {
            ...
            Object.keys(run).forEach(modeName => modes.push({
                name: modeName,
                run: run[modeName]
            }));
        }
        ...
        fs.readdirSync(fixturesDirectory).forEach(fixtureName => {
            ...
            if (fixtureName[0] === "~") {
                ...
                describe(fixtureName, function () {
                    modes.forEach(mode => {
                        runFixtureTest(mode.name, fixtureDirectory, mode.run, mode.name, context);
                    });
                });
            } else {
    ...
    const snapshot = (actual, opts) => {
        ...
        try {
            ...
            if (updateExpectations) {
                ...
                e.message = `SnapshotError: ${path.relative(process.cwd(), actualPath)}`;
                throw e;
            }
        }
    };
...
function normalize(content, isObject, format) {
    if (isObject) {
        ...
        content = replaceAll(content, projectRoot, "PROJECT_ROOT").replace(/\r?\n$/g, "");
    }

    ...
    function ensurePreservedKeys(lookupA, lookupB, path) {
        ...
        for (var key in lookupB) {
            if (lookupA[key]) {
                ...
                expect(lookupA[key].el === lookupB[key].el, 'unpreserved element "' + fullPath + '"').to.equal(true);
                expect(lookupA[key].component === lookupB[key].component, 'unpreserved component "' + fullPath + '"').to.equal(true);
                ensurePreservedKeys(lookupA[key].children, lookupB[key].children, fullPath);
            }
        }
    }
};
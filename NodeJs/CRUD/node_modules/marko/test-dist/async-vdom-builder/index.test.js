...
describe("AsyncVDOMBuilder", function () {
    ...
    it("async", function (done) {
        ...
        out.on("finish", function (result) {
            ...
            expect(tree.R_.ay_).to.equal("div");
            expect(tree.R_.at_.ay_).to.equal("span");
            expect(tree.R_.at_.at_.ay_).to.equal("section");
            done();
        });
    });
    ...
    it("for loop", function (done) {
        ...
        out.once("finish", function (result) {
            ...
            expect(header.ay_).to.equal("h1");
            expect(list.ay_).to.equal("ul");
            expect(getChildNodes(list).length).to.equal(10);
            expect(paragraph.ay_).to.equal("p");
            done();
        });
    ...
    it("staticNode, text, comment", function (done) {
        ...
        out.once("finish", function (result) {
            var tree = result.getOutput();
            var childNodes = getChildNodes(tree);
            expect(childNodes[0].ay_).to.equal("div");
            expect(childNodes[1].aF_).to.equal("Hello <em>World</em>");
            expect(childNodes[2].aF_).to.equal("TODO: make this work");
            done();
        });
    });

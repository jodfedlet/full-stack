...
function getRootDeps(template, context) {
    if (production && template._W_) {
        return template._W_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template._W_ = Object.keys(template._X_).map(key => template._X_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target._X_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag._X_) Object.assign(deps, tag._X_);
